export default class WordBuilder {
    ...
    _buildWordElementForWordAtIndex(word, index, colorDefiner) {
        if (this.isComment) {
            return `<span style="font-size: 24px; color: gray; white-space: pre; height: 100%;">${word}</span>`
        }
        else {
            this.color = colorDefiner.defineColorForWordAtIndex(word, index)
            return `<span style="font-size: 24px; min-height:28.8px; height: 100%; color: ${this.color}; white-space: pre;">${word}</span>`
        }
    }
}
